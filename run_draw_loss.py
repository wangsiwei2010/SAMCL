...
parser.add_argument('--alpha_inter', type=float, default=0.1)
...
def draw_loss_distribution(file_path, epoch):
    data = np.loadtxt(file_path)

    label = data[:, 0]
    subgraph_loss = data[:, 1]
    loss_normal = subgraph_loss[label == 0]
    loss_abnormal = subgraph_loss[label == 1]
    # pos_nor = np.unique( np.where(data == 0)[0] )
    # pos_abnor = np.unique( np.where(data == 1)[0] )
    # loss_normal = data[pos_nor][:, 1].tolist()
    # loss_abnormal = data[pos_abnor][:, 1].tolist()

    normal_mean = np.mean(loss_normal)
    abnormal_mean = np.mean(loss_abnormal)

    plt.figure()
    x_lables = ['Normal', 'Abnormal']
    n, bins, patches = plt.hist([loss_normal, loss_abnormal], bins=50, color=['green', 'red'], stacked=False, label=x_lables, alpha=0.6)
    # 拟合一条最佳正态分布曲线
    y_nor = norm.pdf(bins, np.mean(loss_normal), np.std(loss_normal))
    y_abnor = norm.pdf(bins, np.mean(loss_abnormal), np.std(loss_abnormal))
    plt.plot(bins, y_nor, 'g--')
    plt.plot(bins, y_abnor, 'r--')
    plt.legend(loc='upper left')
    plt.title('Loss_' + str(epoch))
    save_dir = 'loss_fig/'
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(save_dir + 'loss_' + str(epoch))

    return normal_mean, abnormal_mean



# def distrib_tester(file_path, epoch, bins2=50, sampling=0.):
#     data = np.loadtxt(file_path)
#
#     pos_nor = np.unique( np.where(data == 0)[0] )
#     pos_abnor = np.unique( np.where(data == 1)[0] )
#     distrib1 = data[pos_nor][:, 1].tolist()
#     distrib2 = data[pos_abnor][:, 1].tolist()
#
#     # sampling
#     if sampling > 0:
#         idx = np.random.permutation(len(distrib1))
#         distrib1 = distrib1[idx[:int(sampling*len(distrib2))]]
#
#     interval = np.max(distrib2, axis=0) - np.min(distrib2, axis=0)
#     step = interval / np.float32(bins2)
#     bins1 = np.int32(np.round((np.max(distrib1) - np.min(distrib1)) / (step + 1.e-10)))
#     if bins1 == 0:
#         bins1 = 1
#
#     fig = plt.figure()
...
#     ax1.hist(distrib1, bins=bins1, color='blue', alpha=0.5, label='Normality')
#     ax2.hist(distrib2, bins=bins2, color='m', alpha=0.5, label='Abnormality')
#
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            for batch_idx in range(batch_num):
                ...
                score_ot_try = - (ot_1 + ot_2) / 2
                score_ot_try = score_ot_try.cpu().numpy()
                ...
                ano_score_ot_try = scaler4.fit_transform(score_ot_try.reshape(-1, 1)).reshape(-1)

                ano_scores = ano_score_co + alpha_recon * ano_score_re + alpha_inter * ano_score_ot_try

                loss[idx] = ano_scores

            save_dir = 'loss/'
            os.makedirs(save_dir, exist_ok=True)
            loss_path = save_dir + args.dataset + '_loss_' + str(epoch + 1) + '.txt'
            f = open(loss_path, 'w')
            for i in range(nb_nodes):
                f.write(str(ano_label[i]) + '   ' + str(loss[i]) + '\n')
            f.close()
            normal_mean, abnormal_mean = draw_loss_distribution(loss_path, (epoch + 1))
            print('Normal:{:.4f}  Abnormal:{:.4f}  Diff:{:.4f}'.format(normal_mean, abnormal_mean, (abnormal_mean - normal_mean)))
